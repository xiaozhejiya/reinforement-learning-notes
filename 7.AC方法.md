...
<img src="./image/image-20260827213926005.png" alt="image-20260827213926005" style="zoom:50%;" />
...
<img src="./image/image-20260828005912806.png" alt="image-20260828005912806" style="zoom:50%;" />

### deterministic policies

因为我们之前的策略是会保证每个$action$的概率>0的，这个就只能用于$action$是有限个的情况下

如果$action$是无限的，那么就需要使用deterministic policies

我们先定义
$$
a=\mu(s,\theta)\stackrel{\cdot}{=}\mu(s)
$$
这里$\mu$(参数为$\theta$)是一个函数输入是$s$，$\mu$输出不是在$s$采取$a$的概率，他的输出直接是$a$

- $\mu$是一个把状态空间$\mathcal{S}$映射到$\mathcal{A}$的函数
- 我们通常会把$\mu(s,\theta)$写成$\mu(s)$
...
$$
如何选择$d_0$？比较简单的例子就是直接选取与$\mu$无关的策略

- case 1:我们只关心某一个状态，那么我们可以直接令$d_0(s_0)=1和d_0(s\neq s_0)=0$这时目标函数变为$v_\pi(s_0)$
- case 2:$d_0$是一个来自策略$\beta$稳态分布
...
\right|_{a=\mu(s')}
\\
&\stackrel{\cdot}{=} \sum_{s'\in\mathcal S}
\rho_\mu(s')
\nabla_\theta\mu(s')
\left.
\left(\nabla_a q_\mu(s',a)\right)
...
\end{align}
$$
有一个重要的公式$v_\mu(s)=q_\mu(s,\mu(s))$，直观上理解就是我既然只能选择某一个策略了($\pi(a_0|s)=1$)，那么求期望时就只剩那个策略对应的q了

$$
\sum_a\pi(a|s)\underbrace{\bigg[\sum_rp(r|s,a)r+\gamma\sum_{s'}v_\pi(s')p(s'|a,s)\bigg]}_{q(s,a)}
$$
此时$v_\pi(s)=q_\pi(s,a_0)$

于是我们就可以对$\sum_{s\in\mathcal S} d_0(s)\nabla_\theta v_\mu(s)$中的$v_\mu(s)$进行替换，得到：
$$
\sum_{s\in\mathcal S} d_0(s)\nabla_\theta q_\mu(s,a)
\\
\begin{align}
\\其中\nabla_\theta q_\mu(s,a)&=\nabla_\theta q_\mu(s,\mu(s,\theta)),在通过链式求导得到\nabla_\theta\mu(s')
\left.
\left(\nabla_a q_\mu(s',a)\right)
\right|_{a=\mu(s')}
\\&=\sum_{s\in\mathcal S}
...
<img src="./image/image-20260828163227489.png" alt="image-20260828163227489" style="zoom: 33%;" />
