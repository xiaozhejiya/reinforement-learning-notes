...
\\表示现在的value和target之间存在一个误差\delta_t,称为\text{TD error}
...
\\我们只是把v替换成了q,为什么要进行替换呢?
\\
$$

**用$V(s)$做决策**

假设智能体处于起点$S_0$,面前有左右两个动作$A_l,A_r$.如果智能体手中只有$v(s)$表格,我们就需要考虑选$A_l$有多大概率转移到状态$S_a$有多大概率到$S_b$

如果选$A_r$又会掉进那个状态.所以即使知道了所有状态的$v(s)$,只要不知道状态转移矩阵,我们依然不知道该怎么走.

自然而然的我们会想,既然不知道状态转移矩阵,那么我们通过采样去试一下每个动作$a$转移到状态$s$的概率不就行了.没错!当你在状态$s$试着采取动作$a$,拿到真实的奖励$r$,并观察到下一个真实的转移状态$s'$,其实就是在计算:
$$
r+\gamma v(s')
$$
用这个采样得到的值，去评估“在状态 $s$ 下选动作 $a$ 的期望回报”。

**这正是最核心的转变所在：** 当你开始通过采样去评估“在特定状态下，选特定动作的回报”时，你真正在估计的就已经**不再是状态价值 $v(s)$，而是动作价值 $q(s,a)$ 了！**
$$
\textcolor{red}{q_{t+1}(s_t,a_t)=q_t(s_t,a_t)-\alpha_t(s_t,a_t)\bigg[q_t(s_t,a_t)-(r_{t+1}+\gamma q_{t}(s_{t+1},a_{t+1}))\bigg]}
